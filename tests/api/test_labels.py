...
def test_get_label(client, label_test_fixture):
    ...
    response = client.get(f"/api/detection/labels/{label_test_fixture.id}")
...
def test_list_labels(client):
    ...
    response = client.post("/api/detection/labels", json=label_data)

    label_data = {"name": "label_test_fixture2", "color": "#00FF10", "function_id": 1}
    response = client.post("/api/detection/labels", json=label_data)

    response = client.get("/api/detection/labels")
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 2
...
def test_create_label_form(client):
    ...
    response = client.post("/api/detection/labels-form", data=form_data)
...
def test_update_label(client, test_session):
    ...
    response = client.put(f"/api/detection/labels/{label.id}", json=update_data)
    assert response.status_code == 200
    data = response.json()
...
def test_update_label_not_found(client):
    ...
    update_data = {"name": "updated_label", "color": "#0000FF"}
    response = client.put("/api/detection/labels/999", json=update_data)
...
def test_update_label_duplicate_name(client, test_session):
    ...
    response = client.put(f"/api/detection/labels/{label1.id}", json=update_data)
    assert response.status_code == 400
    assert "Label with this name already exists" in response.json()["detail"]
...
def test_delete_label(client, label_test_fixture):
    ...
    response = client.delete(f"/api/detection/labels/{label_test_fixture.id}")
...
def test_delete_label_in_use(client, test_session):
    ...
    test_session.commit()
    response = client.delete(f"/api/detection/labels/{label.id}")
    assert response.status_code == 204
    response = client.get("/api/detection/labels")
    assert response.status_code == 200
    data = response.json()
...
def test_delete_label_not_found(client):
    ...
    response = client.delete("/api/detection/labels/999")
    assert response.status_code == 404
